<x-app-layout>
    ...
    <x-slot name="header">
        <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
            ...
            <x-nav-link class="font-semibold text-lg" 
            :href="route('zone')" :active="request()->routeIs('zone')">
                {{ __('Zones') }}
            </x-nav-link>
        </div>
    </x-slot>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="flex justify-end mx-6">
                    <form action="{{route('church.create')}}" method="get" class="my-5 d-flex justify-content-end">
                        <x-primary-button>{{ __('Create New zone') }}</x-primary-button>
                    </form>
                </div>
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <table class="table-auto w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                       ...
                       <tbody>
                            ...
                            <tr class=" whitespace-nowrap odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                                ...
                                @superadmin
                                    <td class="px-6 py-4">
                                            ...
                                            <form action="{{route('make.admin',['user_id'=>$zone['id']])}}" method="POST">
                                                @csrf
                                                @method('patch')
                        ...
                        @empty
                            <tr class=" whitespace-nowrap odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                                <td class="px-6 py-4 whitespace-nowrap"> {{ __('No zones Found') }} </td>
                            </tr>
                        @endforelse
                       </tbody>
                    </table>
